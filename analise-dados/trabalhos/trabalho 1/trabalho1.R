...
menor <- rowMins(notas)
medquad <- round(sqrt((rowSums(notas^2) - menor^2) / 3), 2)
...
mp1 <- round(mean(alunos[!reprovados, "p1"]), 1)
mp2 <- round(mean(alunos[!reprovados, "p2"]), 1)
mp3 <- round(mean(alunos[!reprovados, "p3"]), 1)
mp4 <- round(mean(alunos[!reprovados, "p4"]), 1)
...
dp1 <- round(sd(alunos[!reprovados, "p1"]), 1)
dp2 <- round(sd(alunos[!reprovados, "p2"]), 1)
dp3 <- round(sd(alunos[!reprovados, "p3"]), 1)
dp4 <- round(sd(alunos[!reprovados, "p4"]), 1)
...
dminCamp <- leituras[leituras$cidade == "Campinas" & leituras$chuva == min(leituras[leituras$cidade == "Campinas", c("chuva")]), c("dia")][1]
