...
Q1 <- setdiff(C, L); Q1 # "Xampu" "Sabonete" "Refrigerante" "Queijo" "Suco" "Vinho"

## Quais os produtos que sao vendidos em Vinhedo, mas nao sao vendidos em Campinas?
Q2 <- setdiff(V, C); Q2 # "Detergente" "Carne" "Papel" "Iogurte"

## Quais os produtos que sao vendidos em pelo menos uma cidade?
Q3 <- union(union(C, L), V); Q3 # "Xampu" "Sabonete" "Arroz" "Chocolate" "Leite" "Refrigerante" "Queijo" "Suco" "Vinho" "Cerveja" "Detergente" "Carne" "Papel" "Iogurte"

## Quais os produtos que sao vendidos em todas as cidades?
Q4 <- intersect(intersect(C, L), V); Q4 # "Chocolate" "Leite"
...
# all(L %in% C) # RETORNA TRUE POIS VE QUE TODOS ITENS DE L ESTA EM C
Q5 <- setdiff(L, C) # RETORNA character(0) INDICA QUE OS PRODUTOS DE L ESTA EM C
...
alunos <- data.frame(ids = ids, p1 = p1, p2 = p2, p3 = p3, p4 = p4); alunos
...
notas <- as.matrix(alunos[, c("p1", "p2", "p3", "p4")]); notas
menor <- rowMins(notas); menor # USANDO A FUNCAO rowMins()
media_final <- sqrt((rowSums(notas^2) - menor^2) / 3); media_final
# -> ( 8.75, 7.04, 8.10, 7.85, 8.13, 6.26, 0.00, 5.49, 3.31, 8.47, 7.05, 7.01, 5.35, 7.53, 2.99, 6.65, 6.53, 7.72, 9.33, 8.20, 6.62, 8.16, 8.44, 1.12, 8.22, 9.63 ,6.74 7,.76 6.,80 8.7,0 3.31, 8.66 ,9.07 8,.74 1.,85 8.6,0, 8.41, 5.16 8,.22 8.9,0 6.68 ,8.04 6.,87 7.79, 3.73 8,.54 7.8,5 7.9,0, 7.15, 4.29, 4.89)
alunos$media_final <- round(media_final, 2); alunos
medquad <- alunos$media_final; medquad
...
alunos$reprovados <- alunos$media_final < 7; alunos
reprovados <- alunos$reprovados; reprovados
...
aprovados <- alunos[!alunos$reprovados, ]; aprovados

mp1 <- mean(aprovados$p1); mp1
mp2 <- mean(aprovados$p2); mp2
mp3 <- mean(aprovados$p3); mp3
mp4 <- mean(aprovados$p4); mp4
...
dp1 <- sd(aprovados$p1); dp1
dp2 <- sd(aprovados$p2); dp2
dp3 <- sd(aprovados$p3); dp3
dp4 <- sd(aprovados$p4); dp4
...
dmaxCamp <- (leituras[leituras$cidade == "Campinas" & leituras$chuva == max(leituras$chuva[leituras$cidade == "Campinas"]), c("dia")])[1]; dmaxCamp
dmaxLim <- (leituras[leituras$cidade == "Limeira" & leituras$chuva == max(leituras$chuva[leituras$cidade == "Limeira"]), c("dia")])[1]; dmaxLim
dmaxVin <- (leituras[leituras$cidade == "Vinhedo" & leituras$chuva == max(leituras$chuva[leituras$cidade == "Vinhedo"]), c("dia")])[1]; dmaxVin
...
dminCamp <- (leituras[leituras$cidade == "Campinas" & leituras$chuva == min(leituras$chuva[leituras$cidade == "Campinas"]), c("dia")])[1]; dminCamp
dminLim <- (leituras[leituras$cidade == "Limeira" & leituras$chuva == min(leituras$chuva[leituras$cidade == "Limeira"]), c("dia")])[1]; dminLim
dminVin <- (leituras[leituras$cidade == "Vinhedo" & leituras$chuva == min(leituras$chuva[leituras$cidade == "Vinhedo"]), c("dia")])[1]; dminVin
