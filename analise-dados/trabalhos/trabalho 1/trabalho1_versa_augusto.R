...
# qual é o retorno esperado nessa questão?
...
enc <- as.numeric((bin | key) & !(bin & key))
...
enc_dec <- sum(enc * (2 ^((length(enc)-1):0)))
...
alunos_min_0 = alunos
comparacao <- alunos[, c(2:5)] == rowMins(alunos[, c(2:5)])
alunos_min_0[, c(2:5)][comparacao] <- 0
medquad <- round(sqrt(rowSums(alunos_min_0[, c(2:5)]^2) / 3), 2)

notas <- as.matrix(alunos[, c("p1", "p2", "p3", "p4")])
menor <- rowMins(notas)
medquad <- round(sqrt((rowSums(notas^2) - menor^2) / 3), 2); media_final
...
mp1 <- mean(alunos[!reprovados, "p1"])
mp2 <- mean(alunos[!reprovados, "p2"])
mp3 <- mean(alunos[!reprovados, "p3"])
mp4 <- mean(alunos[!reprovados, "p4"])
...
dp1 <- sd(alunos[!reprovados, "p1"])
dp2 <- sd(alunos[!reprovados, "p2"])
dp3 <- sd(alunos[!reprovados, "p3"])
dp4 <- sd(alunos[!reprovados, "p4"])
...
acumCamp <- sum(leituras[leituras$cidade == "Campinas", c("chuva")])
acumLim <- sum(leituras[leituras$cidade == "Limeira", c("chuva")])
acumVin <- sum(leituras[leituras$cidade == "Vinhedo", c("chuva")])
...
dmaxCamp <- leituras[leituras$cidade == "Campinas" & leituras$chuva == max(leituras[leituras$cidade == "Campinas", c("chuva")]), c("dia")][1]
dmaxLim <- leituras[leituras$cidade == "Limeira" & leituras$chuva == max(leituras[leituras$cidade == "Limeira", c("chuva")]), c("dia")][1]
dmaxVin <- leituras[leituras$cidade == "Vinhedo" & leituras$chuva == max(leituras[leituras$cidade == "Vinhedo", c("chuva")]), c("dia")][1]
...
dminCamp <- leituras[leituras$cidade == "Campinas" & leituras$chuva == min(leituras[leituras$cidade == "Campinas", c("chuva")]), c("dia")][1]; dminCamp
dminLim <- leituras[leituras$cidade == "Limeira" & leituras$chuva == min(leituras[leituras$cidade == "Limeira", c("chuva")]), c("dia")][1]
dminVin <- leituras[leituras$cidade == "Vinhedo" & leituras$chuva == min(leituras[leituras$cidade == "Vinhedo", c("chuva")]), c("dia")][1]
