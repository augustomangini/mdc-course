...
# 9) Adicionando colunads de tempo para auxiliar nas análises
...
write.csv2(tabela, "tabela_pandemia.csv", row.names = FALSE)
...
cepagri$hora <- as.numeric(format(cepagri$horario, "%H"))
cepagri$minuto <- as.numeric(format(cepagri$horario, "%M"))
cepagri$data <- format(cepagri$horario, "%d%m%Y")

encontrarSequencias <- function(cepagri, limiar) {
  cepagriNoite=cepagri[(cepagri$hora > 18 | cepagri$hora<6),]
  sequencialAtual = 0
  sequenciaData = c()
  sequenciaQtde = c()
  dataInicio = cepagriNoite$data[1]
  for(i in unique(cepagriNoite$data)) {
    if(all(cepagriNoite$temperatura[cepagriNoite$data == i] > limiar)) {
      sequencialAtual = sequencialAtual+1
    } else {
      if(sequencialAtual>1) {
        sequenciaData = append(sequenciaData, dataInicio)
        sequenciaQtde = append(sequenciaQtde, sequencialAtual)
      }
      sequencialAtual = 0
      dataInicio = i
    }
  ...
  sequencias = data.frame(data = sequenciaData, totalDias = sequenciaQtde)
...
resultados = data.frame(
  limiar = limiares,
  dataInicio = as.Date(character(length(limiares))),
  totalDias = integer(length(limiares))
)

for(i in 1:length(limiares)) {
  # Encontrar sequências para o limiar atual
  seq = encontrarSequencias(cepagri, limiares[i])
  
  # Se encontrou alguma sequência, pegar a maior
  if(nrow(seq) > 0) {
    maior = seq[which.max(seq$totalDias), ]
    resultados$dataInicio[i] = as.Date(maior$data, format = "%d%m%Y")
    resultados$totalDias[i] = maior$totalDias
  } else {
    # Se não encontrou nenhuma sequência
    resultados$dataInicio[i] = NA
    resultados$totalDias[i] = 0
  }
}

library(ggplot2)
# GRÁFICO BÁSICO PARA AVERIGUAR AS SEQUENCIAS
p <- ggplot(resultados, aes(x = limiar, y = totalDias)) +
  geom_line(color = "blue", size = 1) +
  
  geom_point(color = "red", size = 3) +
  geom_text(aes(label = totalDias), vjust = -0.5, size = 3) +
  labs(
    title = "Maior Sequência de Noites Quentes por Limiar",
    x = "Limiar de Temperatura (°C)",
    y = "Maior Sequência (dias)"
  ) +
  theme_minimal() +
  scale_x_continuous(breaks = limiares) +
  scale_y_continuous(breaks = seq(0, max(resultados$totalDias), by = 2))

ggsave("grafico_noites_quentes.png", plot = p, width = 10, height = 6, dpi = 300)

# CONSTRUIR UM GRÁFICO DA TEMPERATURA MÉDIA MENSAL POR ANO
anos_arr   <- c()
meses_arr  <- c()
medias_arr <- c()

# CONSTRUINDO A MÉDIA MENSAL POR ANO
for (a in unique(cepagri$ano)) {
  for (m in 1:12) {
    subset <- cepagri[cepagri$ano == a & cepagri$mes == m, ]
    media  <- mean(subset$temperatura, na.rm = TRUE)
    
    anos_arr   <- c(anos_arr,  a)
    meses_arr  <- c(meses_arr, m)
    medias_arr <- c(medias_arr, media)
  }
}

dados_calor <- data.frame(ano = anos_arr, mes = meses_arr, temp_media = medias_arr)

# CATEGORIAS PARA O GRÁFICO
dados_calor$ano <- factor(dados_calor$ano)
dados_calor$mes <- factor(dados_calor$mes, levels = 1:12, labels = c(
  "Jan","Fev","Mar","Abr","Mai","Jun",
  "Jul","Ago","Set","Out","Nov","Dez"
))

# CONSTRUINDO O GRÁFICO
ggplot(dados_calor, aes(x = mes, y = temp_media, color = ano, group = ano)) +
  geom_line(size = 1.2) +
  geom_point(size = 2.5) +
  scale_color_manual(values = c(
    "#e6194b", "#3cb44b", "#ffe119", "#4363d8", "#f58231",
    "#911eb4", "#42d4f4", "#f032e6", "#bfef45", "#fabed4", "#000075"
  ), name = "Ano") +
  labs(
    title    = "Temperatura Média Mensal por Ano",
    subtitle = "Comparativo 2015–2025",
    x        = "Mês",
    y        = "Temperatura Média (°C)"
  ) +
  theme_minimal() +
...
# SALVAR
ggsave("temperatura_mensal_2015_2025.png", width = 12, height = 6, dpi = 300)